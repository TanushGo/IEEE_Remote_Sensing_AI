...
def restitch_and_plot(options, datamodule, model, parent_tile_id, satellite_type="sentinel2", rgb_bands=[4, 3, 2],
    ...
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list("Settlements",
                                                               ...
                                                               N=4)
    # setting up figure axis plotting
    fig, axs = plt.subplots(nrows=1, ncols=3, figsize=(20, 20))

    # calling restitch eval on the parent tile
    restitched_img, stitched_gt, stitched_pred = restitch_eval(options.processed_dir,
                                                               satellite_type, parent_tile_id, range_x=(0, 4),
                                                               range_y=(0, 4), datamodule=datamodule, model=model)

    # plot the satellite image
    axs[0].set_title(f"{parent_tile_id} restitched")
    axs[0].imshow(np.dstack([restitched_img[0, rgb_bands[0], :, :], restitched_img[0, rgb_bands[1], :, :],
                             restitched_img[0, rgb_bands[2], :, :]]))
...
def restitch_eval(dir: str | os.PathLike, satellite_type: str, tile_id: str, range_x: Tuple[int, int],
    ...
    # The predictions are reshaped and processed (using np.argmax to select the most likely class for each pixel)
    ...
    # returns the stitched satellite image, the stitched ground truth, and the stitched predictions
